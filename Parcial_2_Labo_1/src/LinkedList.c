...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this!=NULL&&nodeIndex>-1&&nodeIndex<=ll_len(this)&&nodeNuevo!=NULL){
		...
		if(nodeIndex==0){
			...
			nodeNuevo->pNextNode=nodeAnterior->pNextNode;
...
int ll_containsAll(LinkedList* this,LinkedList* this2)
{
    ...
    if(this!=NULL&&this2!=NULL){
    	...
    	for(int i=0;i<ll_len(this2);i++){
    		if(!ll_contains(this, ll_get(this2, i))){ // Los nodos son bellos.
    			returnAux=0;
    			break;
    		}
    	}
...
{
    int returnAux =-1;
    void* auxElementActual=NULL;
	void* auxElementAnterior=NULL;
	int flagOrdenar=1;
    if(this!=NULL&&pFunc!=NULL&&(order==1||order==0)){
    	while(flagOrdenar==1){
    		flagOrdenar=0;
    		for(int j=1;j<ll_len(this);j++){
    			auxElementActual=ll_get(this, j);
    			auxElementAnterior=ll_get(this, j-1);
    			if((order&&pFunc(auxElementActual, auxElementAnterior)<0)||(!order&&pFunc(auxElementActual, auxElementAnterior)>0)){
					ll_set(this, j, auxElementAnterior);
					ll_set(this, j-1, auxElementActual);
					flagOrdenar=1;
    			}
    		}
    	}
